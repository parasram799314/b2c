...
export default function FlightsTab({ rfq, planItems, onAddToPlan }) {
  ...
  const [toCity,   setToCity]   = useState(firstDest?.destination || '');
      ...
      <div style={{flexShrink:0,background:'linear-gradient(135deg,#F7BE39,#c99a20)',padding:'10px'}}>
        <div style={{background:'#fff',borderRadius:'12px',padding:'14px',boxShadow:'0 4px 16px rgba(0,0,0,0.1)'}}>
          ...
          <div style={{marginBottom:'8px'}}>
            ...
            <div style={inputBox}><span style={{color:'#F7BE39'}}>✈</span><input value={fromCity} onChange={e=>setFromCity(e.target.value)} style={{flex:1,border:'none',background:'transparent',fontSize:'13px',fontWeight:600,color:'#111827',outline:'none'}} /></div>
          </div>

          {/* TO */}
          <div style={{marginBottom:'8px'}}>
            <div style={lbl}>To</div>
            <div style={inputBox}><span>🏙️</span><input value={toCity} onChange={e=>setToCity(e.target.value)} style={{flex:1,border:'none',background:'transparent',fontSize:'13px',fontWeight:600,color:'#111827',outline:'none'}} /></div>
          </div>
