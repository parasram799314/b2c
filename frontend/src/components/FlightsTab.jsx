// components/FlightsTab.jsx — FULL UPDATED FILE
...
// ── Global cities fallback list (same as RFQForm) ─────────────────────────────
const GLOBAL_CITIES = [
  ...
  'Goa, India','Kochi, India','Bhopal, India','Nagpur, India',
  'London, UK','Dubai, UAE','Singapore','New York, USA',
  'Paris, France','Tokyo, Japan','Bangkok, Thailand','Kuala Lumpur, Malaysia',
];

// ── CityInput — type to search + dropdown (from RFQForm pattern) ──────────────
function CityInput({ value, onChange, placeholder, rfqCities = [], otherCity = '' }) {
  const [open, setOpen]     = useState(false);
  const [dropPos, setDropPos] = useState({ top:0, left:0, width:220 });
  const inputRef = useRef();
  const wrapRef  = useRef();

  // Merge rfq cities at top, then global list, deduplicate, exclude otherCity
  const allOptions = useMemo(() => {
    const merged = [...rfqCities, ...GLOBAL_CITIES].filter(
      (c, i, arr) => arr.indexOf(c) === i && c !== otherCity
    );
    return merged;
  }, [rfqCities, otherCity]);
  ...
  const filtered = useMemo(() => {
    ...
    return allOptions.filter(c => c.toLowerCase().includes(q)).slice(0, 8);
  }, [value, allOptions]);
  ...
  const handleFocus = () => {
    if (inputRef.current) {
      ...
      setDropPos({ top: r.bottom + window.scrollY + 4, left: r.left + window.scrollX, width: Math.max(220, r.width) });
    }
    setOpen(true);
  };
  ...
  return (
    <div ref={wrapRef} style={{ position:'relative', flex:1, minWidth:0 }}>
      <input
        ...
        autoComplete="off"
        style={{ width:'100%', border:'none', background:'transparent', outline:'none', fontSize:'13px', fontWeight:600, color:'#111827' }}
      />
      {open && filtered.length > 0 && (
        <div style={{
          position:'fixed', top:dropPos.top, left:dropPos.left, width:dropPos.width,
          zIndex:999999, background:'#fff', border:'1px solid #e5e7eb',
          borderRadius:'12px', boxShadow:'0 12px 32px rgba(0,0,0,0.15)',
          padding:'4px 0', maxHeight:'240px', overflowY:'auto'
        }}>
          {/* RFQ cities shown with a subtle separator if present */}
          {rfqCities.filter(c => c !== otherCity && (value ? c.toLowerCase().includes(value.toLowerCase()) : true)).length > 0 && (
            <div style={{ padding:'4px 12px 2px', fontSize:'9px', fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.07em' }}>
              Trip Cities
            </div>
          )}
          {filtered.map((c, i) => {
            const isTripCity = rfqCities.includes(c);
            return (
              <div
                key={c}
                ...
                style={{
                  ...
                  color:'#1f2937', fontWeight: isTripCity ? 600 : 400,
                  borderTop: i > 0 && isTripCity !== rfqCities.includes(filtered[i-1]) ? '1px solid #f3f4f6' : 'none',
                }}
                onMouseEnter={e => e.currentTarget.style.background='#FFF3DC'}
                onMouseLeave={e => e.currentTarget.style.background='transparent'}
              >
                <span style={{ color: isTripCity ? '#F7BE39' : '#9ca3af', fontSize:'14px', flexShrink:0 }}>
                  📍
                </span>
                <span>{c}</span>
                {isTripCity && (
                  <span style={{ marginLeft:'auto', fontSize:'9px', fontWeight:700, color:'#F7BE39', background:'#FFF3DC', borderRadius:'4px', padding:'1px 5px' }}>
                    TRIP
                  </span>
                )}
              </div>
        ...
        <div style={{padding:'4px 14px 10px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          {inPlan
            ...
            : <button
                onClick={() => onAdd({
                  ...flight,
                  price: flight.currency === 'USD' ? Math.round(Number(flight.price) * 83) : Number(flight.price),
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'18px',width:'100%',maxWidth:'400px',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}}>
            ...
            <div style={{padding:'18px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'14px'}}>
                ...
                <div>
                  <div style={{fontWeight:700,fontSize:'14px',color:'#111827'}}>{flight.airline}</div>
                  <div style={{fontSize:'11px',color:'#9ca3af'}}>{flight.flightNumber}</div>
                </div>
              </div>
              <div style={{background:'#f9fafb',borderRadius:'10px',padding:'14px',display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'12px'}}>
                <div>
                  <div style={{fontSize:'9px',color:'#9ca3af'}}>Departure</div>
                  <div style={{fontSize:'20px',fontWeight:800,color:'#111'}}>{depTime}</div>
                  <div style={{fontSize:'12px',fontWeight:600,color:'#374151'}}>{flight.from||flight.fromAirport}</div>
                </div>
                <div style={{textAlign:'center'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',margin:'4px 0'}}>
                    ...
                    <span style={{margin:'0 3px',fontSize:'11px'}}>✈</span>
                    <div style={{width:'28px',height:'1.5px',background:'#d1d5db'}}/>
                  </div>
                  <div style={{fontSize:'10px',fontWeight:700,color:'#16a34a'}}>{nonStop?'Non-stop':`${stops} Stop(s)`}</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:'9px',color:'#9ca3af'}}>Arrival</div>
                  <div style={{fontSize:'20px',fontWeight:800,color:'#111'}}>{arrTime}</div>
                  <div style={{fontSize:'12px',fontWeight:600,color:'#374151'}}>{flight.to||flight.toAirport}</div>
                </div>
              </div>
...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function FlightsTab({ rfq, planItems, onAddToPlan }) {
  ...
  // ── RFQ cities ────────────────────────────────────────────────────────────
  const sourceCity      = rfq?.sourceCity || rfq?.source || 'Indore, India';
  const rfqDestinations = rfq?.destinations?.map(d => d.destination).filter(Boolean) || [];
  const rfqCities       = [sourceCity, ...rfqDestinations].filter((c, i, arr) => arr.indexOf(c) === i);

  const hasReturnToBase = !!(rfq?.returnToBase || rfq?.return_to_base || rfq?.returnToSource);
  const adults          = rfq?.numberOfPax || rfq?.numberOfAdults || 1;
  const firstDest       = rfq?.destinations?.[0];

  const [fromCity, setFromCity] = useState(sourceCity);
  const [toCity,   setToCity]   = useState(rfqDestinations[0] || '');
  const [tripType, setTripType] = useState('oneWay');

  // Reset tripType if returnToBase removed
  useEffect(() => { if (!hasReturnToBase) setTripType('oneWay'); }, [hasReturnToBase]);

  const baseDate = firstDest?.dateOfArrival
    ? new Date(firstDest.dateOfArrival).toISOString().slice(0,10)
  ...
  useEffect(() => {
    ...
    const strip = [];
    for (let i = -2 + dateOffset; i <= 2 + dateOffset; i++) {
      const d = new Date(base); d.setDate(d.getDate() + i);
      if (d >= today) strip.push(d.toISOString().slice(0,10));
    }
    setDates(strip);
    if (!selectedDate || !strip.includes(selectedDate))
  ...
  const doSearch = async (from, to, date) => {
    ...
    try {
      ...
      setRawFlights(list);
      if (list.length) {
        const prices = list.map(f => Number(f.price)).filter(p => p > 0);
        if (prices.length) {
          const mn = Math.min(...prices), mx = Math.max(...prices);
          setMinPrice(mn); setMaxPriceLimit(mx);
          setFilters(p => ({ ...p, maxPrice: mx }));
        }
      }
    } catch { setRawFlights([]); }
    setLoading(false);
  ...
  const airlineStats = useMemo(() => {
    ...
    rawFlights.forEach(f => {
      if (!s[f.airline]) s[f.airline] = { count:0, minPrice:Infinity, logo:f.logo, name:f.airline, currency:f.currency||'USD' };
      s[f.airline].count++;
      s[f.airline].minPrice = Math.min(s[f.airline].minPrice, Number(f.price));
    });
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflowY:'auto', background:'#f3f4f6' }}>
        ...
        <div style={{ background:'#fff', borderRadius:'12px', padding:'14px', boxShadow:'0 4px 16px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ display:'flex', gap:'6px', marginBottom:'12px' }}>
            <button onClick={() => setTripType('oneWay')}
              style={{ padding:'5px 14px', borderRadius:'20px', fontSize:'12px', fontWeight:600, cursor:'pointer', background:'transparent', color: tripType==='oneWay'?'#F7BE39':'#6b7280', border: tripType==='oneWay'?'2px solid #F7BE39':'2px solid transparent' }}>
              One Way
            </button>
            {hasReturnToBase && (
              <button onClick={() => setTripType('roundTrip')}
                style={{ padding:'5px 14px', borderRadius:'20px', fontSize:'12px', fontWeight:600, cursor:'pointer', background:'transparent', color: tripType==='roundTrip'?'#F7BE39':'#6b7280', border: tripType==='roundTrip'?'2px solid #F7BE39':'2px solid transparent' }}>
                Round Trip
              </button>
            )}
          </div>

          {/* FROM — CityInput with rfq cities at top */}
          <div style={{ marginBottom:'8px' }}>
            <div style={lbl}>From</div>
            <div style={{ ...inputBox, padding:'6px 10px' }}
              onFocusCapture={e => e.currentTarget.style.borderColor = '#F7BE39'}
              onBlurCapture={e  => e.currentTarget.style.borderColor = '#e5e7eb'}>
              <span style={{ color:'#F7BE39', flexShrink:0 }}>✈</span>
              <CityInput
                value={fromCity}
          ...
          {/* TO — CityInput with rfq cities at top */}
          <div style={{ marginBottom:'8px' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={inputBox}>
                ...
                <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)}
                  style={{ flex:1, border:'none', background:'transparent', fontSize:'12px', fontWeight:600, color:'#111827', outline:'none', cursor:'pointer' }} />
              </div>
            </div>
            <div style={{ flex:1 }}>
              ...
              <div style={inputBox}>
                <span>👤</span>
                <div>
                  <div style={{ fontSize:'12px', fontWeight:600, color:'#111827' }}>{adults} Traveler(s)</div>
                  <div style={{ fontSize:'10px', color:'#9ca3af' }}>Economy</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Search btn */}
          <button
            onClick={() => { setVisibleCount(15); doSearch(fromCity, toCity, selectedDate); }}
            style={{ width:'100%', padding:'11px', background:'#F7BE39', color:'#1a1a1a', border:'none', borderRadius:'10px', fontSize:'14px', fontWeight:800, cursor:'pointer', boxShadow:'0 3px 10px rgba(247,190,57,0.4)' }}>
            Search
          </button>
        </div>
      </div>
        ...
        {showFilters && (
          <div style={{ padding:'0 12px 12px', borderTop:'1px solid #f3f4f6' }}>
            ...
            <div style={{ marginTop:'10px', marginBottom:'10px', display:'flex', alignItems:'center', gap:'8px' }}>
              ...
              <select value={filters.sortBy} onChange={e => setF(p => ({ ...p, sortBy: e.target.value }))}
                style={{ fontSize:'12px', padding:'4px 8px', borderRadius:'8px', border:'1px solid #e5e7eb', color:'#374151', background:'#fff', outline:'none', cursor:'pointer' }}>
            ...
            <div style={{ marginBottom:'10px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'4px' }}>
                ...
                <span style={{ fontSize:'12px', fontWeight:700, color:'#F7BE39' }}>{fmtCurrency(filters.maxPrice, 'INR')}</span>
              </div>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:'10px', color:'#9ca3af' }}>
                <span>{fmtCurrency(minPrice, 'INR')}</span>
                <span>{fmtCurrency(maxPriceLimit, 'INR')}</span>
              </div>
            </div>
      ...
      <div style={{ flexShrink:0, background:'#fff', borderBottom:'1px solid #e5e7eb', padding:'8px 10px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'4px' }}>
          ...
          {dates.map(d => {
            ...
            return (
              <button key={d} onClick={() => { setSelectedDate(d); setVisibleCount(15); }}
                style={{ flex:1, padding:'5px 2px', borderRadius:'8px', cursor:'pointer', border: sel?'2px solid #F7BE39':'1px solid #e5e7eb', background: sel?'#fffdf5':'#fff', textAlign:'center' }}>
      ...
      <div style={{ padding:'10px' }}>
        ...
        {!loading && error && (
          <div style={{ background:'#fee2e2', border:'1px solid #fca5a5', borderRadius:'10px', padding:'12px', color:'#dc2626', fontSize:'12px' }}>⚠️ {error}</div>
        )}
        {!loading && !error && filtered.length === 0 && (
          <div style={{ textAlign:'center', padding:'30px', background:'#fff', borderRadius:'12px', border:'1px solid #e5e7eb' }}>
            ...
            {rawFlights.length > 0 && (
              <button onClick={() => setFilters(mkDefaults(maxPriceLimit))} style={{ marginTop:'10px', padding:'6px 16px', background:'#F7BE39', color:'#1a1a1a', border:'none', borderRadius:'8px', cursor:'pointer', fontWeight:600, fontSize:'12px' }}>Reset Filters</button>
            )}
          </div>
        )}
